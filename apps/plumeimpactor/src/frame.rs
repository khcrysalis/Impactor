use std::{cell::RefCell, path::PathBuf, ptr, rc::Rc, thread};
...
use crate::{
    ...
    pages::{
        DefaultPage, InstallPage, LoginDialog, SettingsDialog, WINDOW_SIZE, WorkPage,
...
impl PlumeFrame {
    pub fn new() -> Self {
        ...
        sizer.add(&work_page.panel, 1, SizerFlag::Expand | SizerFlag::All, 0);
    ...
    fn setup_event_handlers(&mut self) {
        ...
        thread::spawn(move || {
            ...
            let store = rt.block_on(async {
                AccountStore::load(&Some(settings_path))
                    .await
                    .unwrap_or_default()
            });
            tx.send(store).ok();
...
impl PlumeFrame {
    fn bind_widget_handlers(
        ...
        self.settings_dialog.set_remove_handler({
            ...
            move || {
                if let Some(index) = settings_dialog.get_checked_index() {
                    sender
                        .send(PlumeFrameMessage::RequestAccountRemove(index))
                        .ok();
                }
            }
        });
        ...
        self.settings_dialog.set_checklistbox_handler({
            ...
            move |index| {
                sender
                    .send(PlumeFrameMessage::RequestAccountSelect(index))
                    .ok();
            }
        });
        ...
        self.install_page.set_install_handler({
            ...
            move || {
                ...
                binding
                    .plume_frame
                    .install_page
                    .update_fields(&mut signer_settings);
                ...
                let selected_device = if signer_settings.install_mode == SignerInstallMode::Export {
                    ...
                    let Some(device_id) = binding.usbmuxd_selected_device_id.as_deref() else {
                        sender
                            .send(PlumeFrameMessage::Error(
                                "No device selected for installation.".to_string(),
                            ))
                            .ok();
                        return;
                    };
                ...
                let gsa_account_data = if needs_account {
                    match binding.account_store.selected_account() {
                        ...
                        None => {
                            sender
                                .send(PlumeFrameMessage::Error(
                                    "No account selected. Please add an account in Settings."
                                        .to_string(),
                                ))
                                .ok();
                            return;
                ...
                thread::spawn(move || {
                    ...
                    let install_result = rt.block_on(async {
                        ...
                        let device = if let Some(device_id) = selected_device {
                            if device_id == u32::MAX.to_string() {
                                ...
                                if signer_settings.mode == SignerMode::Adhoc {
                                    return Err(
                                        "Ad-hoc signing is not supported for Mac installations."
                                            .to_string(),
                                    );
                                }
                                ...
                                {
                                    return Err("Mac installation not supported on this platform"
                                        .to_string());
                                }
                            } else {
                                Some(
                                    get_device_for_id(&device_id)
                                        .await
                                        .map_err(|_| "Selected device not found".to_string())?,
                                )
                            }
                        } else {
                            None
                        ...
                        sender_clone
                            .send(PlumeFrameMessage::WorkUpdated(
                                "Preparing package for installation...".into(),
                                10,
                            ))
                            .ok();
                        ...
                        match signer_settings.mode {
                            SignerMode::Pem => {
                                let Some(gsa_account) = gsa_account_data else {
                                    return Err("No Apple ID account available for installation."
                                        .to_string());
                                };
                                ...
                                .await
                                .map_err(|e| {
                                    format!("Failed to create developer session: {}", e)
                                })?;
                                ...
                                if teams.is_empty() {
                                    return Err(
                                        "No teams available for the Apple ID account.".to_string()
                                    );
                                }
                                ...
                                let team_id = if teams.len() == 1 {
                                    ...
                                    let (tx, rx) = std::sync::mpsc::channel();
                                    sender_clone
                                        .send(PlumeFrameMessage::RequestTeamSelection(
                                            team_names, tx,
                                        ))
                                        .ok();
                                ...
                                sender_clone
                                    .send(PlumeFrameMessage::WorkUpdated(
                                        "Obtaining certificate identity...".into(),
                                        20,
                                    ))
                                    .ok();
                                ...
                                if let Some(device) = device.as_ref() {
                                    sender_clone
                                        .send(PlumeFrameMessage::WorkUpdated(
                                            "Ensuring current device is registered...".to_string(),
                                            30,
                                        ))
                                        .ok();
                                    session
                                        ...
                                        .await
                                        .map_err(|e| {
                                            format!("Failed to ensure device is registered: {}", e)
                                        })?;
                                }

                                let mut signer =
                                    Signer::new(Some(cert_identity), signer_settings.clone());
                        ...
                        sender_clone
                            .send(PlumeFrameMessage::WorkUpdated("Preparing...".into(), 70))
                            .ok();
                        ...
                        match signer_settings.install_mode {
                            SignerInstallMode::InstallMac => {
                                ...
                                {
                                    if let Some(device) = device.as_ref() {
                                        device.install_app_mac(&package_file).await.map_err(
                                            |e| format!("Failed to install mac app: {}", e),
                                        )?;
                                    } else {
                                        return Err(
                                            "No device found for Mac installation.".to_string()
                                        );
                                    }
                                }
                            }
                            SignerInstallMode::Install => {
                                let progress_callback = {
                                    ...
                                    move |progress: i32| {
                                        ...
                                        async move {
                                            let percent = 70
                                                + ((progress as f32 / 100.0) * 30.0).round() as i32;
                                            sender
                                                .send(PlumeFrameMessage::WorkUpdated(
                                                    format!("Installing... {}%", progress),
                                                    percent,
                                                ))
                                                .ok();
                                        }
                                ...
                                let device = device.ok_or_else(|| {
                                    "No device found for installation.".to_string()
                                })?;

                                device
                                    .install_app(&package_file, progress_callback)
                                    .await
                                ...
                                if signer_settings.app.supports_pairing_file() {
                                    if let Some(usbmuxd_device) = &device.usbmuxd_device {
                                        if usbmuxd_device.connection_type
                                            == idevice::usbmuxd::Connection::Usb
                                        {
                                            if let (
                                                Some(custom_identifier),
                                                Some(pairing_file_bundle_path),
                                            ) = (
                                                ...
                                                device
                                                    ...
                                                    .map_err(|e| {
                                                        format!(
                                                            "Failed to install pairing record: {}",
                                                            e
                                                        )
                                                    })?;
                                            }
                            ...
                            SignerInstallMode::Export => {
                                ...
                                sender_clone
                                    .send(PlumeFrameMessage::ArchivePathReady(archive_path))
                                    .ok();
                            }
                        }
                    ...
                    if let Err(e) = install_result {
                        sender_clone.send(PlumeFrameMessage::WorkEnded).ok();
                        sender_clone
                            .send(PlumeFrameMessage::Error(format!("{}", e)))
                            .ok();
                        return;
                    }
                });
        ...
        self.install_page.set_install_choice_select_handler({
            ...
            move || {
                sender
                    .send(PlumeFrameMessage::InstallButtonStateChanged)
                    .ok();
            }
        });
                    ...
                    move || {
                        ...
                        let account_result = rt.block_on(Account::login(
                            ...
                            || {
                                ...
                                {
                                    return Err(
                                        "Failed to send 2FA request to main thread.".to_string()
                                    );
                                }
                                match code_rx.recv() {
                                    Ok(result) => result,
                                    Err(_) => {
                                        Err("2FA process cancelled or main thread error."
                                            .to_string())
                                    }
                                }
                            },
                        ...
                        match account_result {
                            Ok(account) => {
                                let gsa_account_result = rt.block_on(
                                    plume_core::store::account_from_session(email.clone(), account),
                                );
                                match gsa_account_result {
                                    Ok(gsa_account) => {
                                        sender
                                            .send(PlumeFrameMessage::RequestAccountAdd(gsa_account))
                                            .ok();
                                    }
                                    Err(e) => {
                                        sender
